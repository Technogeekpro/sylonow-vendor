  ...
  bool _validateCurrentStep(AddServiceController controller) {
    ...
    switch (_currentStep) {
      case 0: // Basic Info
        ...
        final hasServiceEnvironment =
            controller.selectedServiceEnvironments.isNotEmpty;

        print(
            '📝 Title filled: $hasTitle ("${controller.titleController.text}")');
        print(
            '📂 Category selected: $hasCategory (${controller.selectedCategory})');
        print(
            '🏠 Service environment: $hasServiceEnvironment (${controller.selectedServiceEnvironments})');
      ...
      case 2: // Pricing
        final hasOriginalPrice =
            controller.originalPriceController.text.isNotEmpty;
        final hasOfferPrice = controller.offerPriceController.text.isNotEmpty;
        final validPricing = _validatePricing(controller);

        print(
            '💰 Original price filled: $hasOriginalPrice ("${controller.originalPriceController.text}")');
        print(
            '💸 Offer price filled: $hasOfferPrice ("${controller.offerPriceController.text}")');
      ...
      case 3: // Details
        ...
        print(
            '⏰ Setup time selected: $hasSetupTime (${controller.selectedSetupTime})');
        print(
            '📅 Booking notice selected: $hasBookingNotice (${controller.selectedBookingNotice})');
        print(
            '📋 Inclusions added: $hasInclusions (${controller.inclusions.length} inclusions: ${controller.inclusions})');
      ...
      case 4: // Area
        ...
        print(
            '📍 Pincodes added: $hasPincodes (${controller.pincodes.length} pincodes: ${controller.pincodes})');
    ...
    try {
      ...
      if (!isFormValid) {
        ...
        final diagnostics = {
          ...
          'title_valid':
              controller.validateTitle(controller.titleController.text) == null,
          'category_selected': controller.selectedCategory != null,
          'service_env_selected':
              controller.selectedServiceEnvironments.isNotEmpty,
          'original_price_filled':
              controller.originalPriceController.text.isNotEmpty,
          'original_price_valid': controller
                  .validatePrice(controller.originalPriceController.text) ==
              null,
          'offer_price_filled': controller.offerPriceController.text.isNotEmpty,
          'offer_price_valid': controller
                  .validateOfferPrice(controller.offerPriceController.text) ==
              null,
        ...
        final failedChecks = diagnostics.entries
            .where((e) => !e.value)
            .map((e) => e.key)
            .toList();
        ...
        _showErrorDialog(
            'Form validation failed. Issues found: ${failedChecks.join(', ')}');
        return;
      }
